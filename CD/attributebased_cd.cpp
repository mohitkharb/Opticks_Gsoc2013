...
#include "ui_attributebased_cd.h"
#include<iostream>
#include<math.h>
AttributeBased_cd::AttributeBased_cd(int row,int col,int **final_codes,int **labels1,int **labels2,int ***data1,int ***data2,Image i1,Image i2,int num_band, QWidget *parent) :
    ...
    ui(new Ui::AttributeBased_cd)
{
    ...
    maxStdRed = (float)INT_MIN;maxStdGreen =(float) INT_MIN; maxStdBlue =(float) INT_MIN;
    minStdRed = (float)INT_MAX ;minStdGreen =(float) INT_MAX;minStdBlue=(float)INT_MAX;
    ...
    int tArea,tPerimeter,tMeanRed,tMeanGreen,tMeanBlue;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        tArea=i2.objectArray[i].fVector.area;
        if(tArea>maxArea)maxArea=tArea;
        if(tArea<minArea)minArea=tArea;

        tPerimeter=i2.objectArray[i].fVector.perimeter;
        if(tPerimeter>maxPerimeter)maxPerimeter=tPerimeter;
        if(tPerimeter<minPerimeter)minPerimeter=tPerimeter;

        tMeanRed=i2.objectArray[i].fVector.meanRed;
        if(tMeanRed>maxMeanRed)maxMeanRed=tMeanRed;
        if(tMeanRed<minMeanRed)minMeanRed=tMeanRed;

        tMeanGreen=i2.objectArray[i].fVector.meanGreen;
        if(tMeanGreen>maxMeanGreen)maxMeanGreen=tMeanGreen;
        if(tMeanGreen<minMeanGreen)minMeanGreen=tMeanGreen;

        tMeanBlue=i2.objectArray[i].fVector.meanBlue;
        if(tMeanBlue>maxMeanBlue)maxMeanBlue=tMeanBlue;
        if(tMeanBlue<minMeanBlue)minMeanBlue=tMeanBlue;

        tStdRed=i2.objectArray[i].fVector.stdRed;
        if(tStdRed>maxStdRed)maxStdRed=tStdRed;
        if(tStdRed<minStdRed)minStdRed=tStdRed;

        tStdGreen=i2.objectArray[i].fVector.stdGreen;
        if(tStdGreen>maxStdGreen)maxStdGreen=tStdGreen;
        if(tStdGreen<minStdGreen)minStdGreen=tStdGreen;

        tStdBlue=i2.objectArray[i].fVector.stdBlue;
        if(tStdBlue>maxStdBlue)maxStdBlue=tStdBlue;
        if(tStdBlue<minStdBlue)minStdBlue=tStdBlue;

        tCompactness=i2.objectArray[i].fVector.compactness;
        if(tCompactness>maxCompactness)maxCompactness=tCompactness;
        if(tCompactness<minCompactness)minCompactness=tCompactness;

        tRoundness=i2.objectArray[i].fVector.roundness;
        if(tRoundness>maxRoundness)maxRoundness=tRoundness;
        if(tRoundness<minRoundness)minRoundness=tRoundness;


        tDirection=i2.objectArray[i].fVector.direction;
        if(tDirection>maxDirection)maxDirection=tDirection;
        if(tDirection<minDirection)minDirection=tDirection;

        tRoughness=i2.objectArray[i].fVector.roughness;
        if(tRoughness>maxRoughness)maxRoughness=tRoughness;
        if(tRoughness<minRoughness)minRoughness=tRoughness;

        tContrast=i2.objectArray[i].fVector.contrast;
        if(tContrast>maxContrast)maxContrast=tContrast;
        if(tContrast<minContrast)minContrast=tContrast;

        tCoarseness=i2.objectArray[i].fVector.coarseness;
        if(tCoarseness>maxCoarseness)maxCoarseness=tCoarseness;
        if(tCoarseness<minCoarseness)minCoarseness=tCoarseness;
...
AttributeBased_cd::~AttributeBased_cd()
{
    delete ui;
    objs.clear();
    objsnew.clear();
...
void AttributeBased_cd::on_lineEdit_2_returnPressed()
{
    rmaxArea_= ui->lineEdit_2->text().toInt();
}

void AttributeBased_cd::on_lineEdit_returnPressed()
{
    rminArea_=ui->lineEdit->text().toInt();

}

void AttributeBased_cd::on_radioButton_3_clicked()
{
    attribute="area";
    att=6;
    int t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.area;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_4_clicked()
{
    attribute="roundness";
    att=10;
    float t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.roundness;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_5_clicked()
{
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.compactness;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_clicked()
{
	flag2 = 0;
    int *flagObjNew = (int *)malloc(sizeof(int)*i2.NO);
    for(int i = 0;i < i2.NO ;i++ )
        flagObjNew[i] = 0;
    //int *flagobjnew = (int*)(mallocsizeof(int)*i2.NO);

    objs.erase(objs.begin(),objs.end());
    pixels.erase(pixels.begin(),pixels.end());
    objsnew.erase(objsnew.begin(),objsnew.end());
    pixelsnew.erase(pixelsnew.begin(),pixelsnew.end());
    ...
    if(flag1==0)
    {
        ...
        if(objs.size() == 0 ){
                ...
                for(int y = 0 ; y < col ; y++){
                    ...
                    for(std::vector<int>::iterator it = objs.begin();it!=objs.end();++it){
                        ...
                        if(tsv ==tx ){
                            ...
                            int newlabel = labels2[x][y];

                            //flagObjNew[newlabel] = 1;
                            if(flagObjNew[newlabel] != 1){
                                objsnew.push_back(newlabel);
                                flagObjNew[newlabel] = 1;
                            }
                        }
                    }
            ...
            for(int x = 0; x < row ; x++){
                for(int y = 0 ; y < col ; y++){
                    int tx = labels2[x][y];
                    for(std::vector<int>::iterator it = objsnew.begin();it!=objsnew.end();++it){
                        int tsv = *it;
                        if(tsv ==tx ){
                            pixelsnew.push_back(index);
                        }
                    }
                    index++;
                }
            ...
            ui->textBrowser->setText("Click Generate Image");
        }
...
void AttributeBased_cd::on_radioButton_2_clicked()
{
    flag1 = 0;
    objs.erase(objs.begin(),objs.end());
    pixels.erase(pixels.begin(),pixels.end());
    ...
    if(flag2==0)
    {
        flag2=1;
        for(int i = 0 ; i < i2.NO ;i++){
            if(i2.objectArray[i].fVector.area >= rminArea_ && i2.objectArray[i].fVector.area <= rmaxArea_ &&
                    i2.objectArray[i].fVector.roundness >= rminRoundness_ && i2.objectArray[i].fVector.roundness <= rmaxRoundness_ &&
                    i2.objectArray[i].fVector.meanRed >= rminMeanRed_ && i2.objectArray[i].fVector.meanRed <= rmaxMeanRed_ &&
                    i2.objectArray[i].fVector.meanGreen >= rminMeanGreen_ && i2.objectArray[i].fVector.meanGreen <= rmaxMeanGreen_ &&
                    i2.objectArray[i].fVector.meanBlue >= rminMeanBlue_ && i2.objectArray[i].fVector.meanBlue <= rmaxMeanBlue_ &&
                    i2.objectArray[i].fVector.stdRed >= rminStdRed_ && i2.objectArray[i].fVector.stdRed <= rmaxStdRed_ &&
                    i2.objectArray[i].fVector.stdGreen >= rminStdGreen_ && i2.objectArray[i].fVector.stdGreen <= rmaxStdGreen_ &&
                    i2.objectArray[i].fVector.stdBlue >= rminStdBlue_ && i2.objectArray[i].fVector.stdBlue <= rmaxStdBlue_ &&
                    i2.objectArray[i].fVector.perimeter >= rminPerimeter_ && i2.objectArray[i].fVector.perimeter <= rmaxPerimeter_ &&
                    i2.objectArray[i].fVector.direction >= rminDirection_ && i2.objectArray[i].fVector.direction <= rmaxDirection_ &&
                    i2.objectArray[i].fVector.contrast >= rminContrast_ && i2.objectArray[i].fVector.contrast <= rmaxContrast_ &&
                    i2.objectArray[i].fVector.coarseness >= rminCoarseness_ && i2.objectArray[i].fVector.coarseness <= rmaxCoarseness_ &&
                    i2.objectArray[i].fVector.roughness >= rminRoughness_ && i2.objectArray[i].fVector.roughness <= rmaxRoughness_ &&
                    i2.objectArray[i].fVector.compactness >= rminCompactness_ && i2.objectArray[i].fVector.compactness <= rmaxCompactness_ )
...
void AttributeBased_cd::on_pushButton_clicked()
{

    ui->textBrowser->setText(" ");
    if(flag1 == 1){
        pain2img *img1= new pain2img(pixels,data1,row,col,num_band,"Time 1");
        img1->show();

        pain2img *img2= new pain2img(pixels,data2,row,col,num_band,"Time 2");
        img2->show();
...
void AttributeBased_cd::on_lineEdit_3_returnPressed()
{
    rminRoundness_= ui->lineEdit_3->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_4_returnPressed()
{
    rmaxRoundness_= ui->lineEdit_4->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_5_returnPressed()
{
    rminCompactness_= ui->lineEdit_5->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_6_returnPressed()
{
    rmaxCompactness_= ui->lineEdit_6->text().toFloat();
}

void AttributeBased_cd::on_radioButton_6_clicked()
{
    attribute="meanred";
    att=1;
    int t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.meanRed;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_lineEdit_7_returnPressed()
{
    rminMeanRed_= ui->lineEdit_7->text().toInt();
}

void AttributeBased_cd::on_lineEdit_8_returnPressed()
{
    rmaxMeanRed_= ui->lineEdit_8->text().toInt();
}

void AttributeBased_cd::on_radioButton_7_clicked()
{
    attribute="meangreen";
    att=2;
    int t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.meanGreen;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_8_clicked()
{
    attribute="meanblue";
    att=3;
    int t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.meanBlue;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_lineEdit_9_returnPressed()
{
    rminMeanGreen_= ui->lineEdit_9->text().toInt();
}

void AttributeBased_cd::on_lineEdit_10_returnPressed()
{
    rmaxMeanGreen_= ui->lineEdit_10->text().toInt();
}

void AttributeBased_cd::on_lineEdit_11_returnPressed()
{
    rminMeanBlue_= ui->lineEdit_11->text().toInt();
}

void AttributeBased_cd::on_lineEdit_12_returnPressed()
{
    rmaxMeanBlue_= ui->lineEdit_12->text().toInt();
}

void AttributeBased_cd::on_radioButton_9_clicked()
{
    attribute="stdred";
    att=4;
    float t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.stdRed;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_10_clicked()
{
    attribute="stdgreen";
    att=5;
    float t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.stdGreen;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_11_clicked()
{
    attribute="stdblue";
    att=6;
    float t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.stdBlue;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_lineEdit_13_returnPressed()
{
    rminStdRed_= ui->lineEdit_13->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_14_returnPressed()
{
    rmaxStdRed_= ui->lineEdit_14->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_15_returnPressed()
{
    rminStdGreen_= ui->lineEdit_15->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_16_returnPressed()
{
    rmaxStdGreen_= ui->lineEdit_16->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_17_returnPressed()
{
    rminStdBlue_= ui->lineEdit_17->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_18_returnPressed()
{
    rmaxStdBlue_= ui->lineEdit_18->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_19_returnPressed()
{
    rminPerimeter_= ui->lineEdit_19->text().toInt();
}

void AttributeBased_cd::on_lineEdit_20_returnPressed()
{
    rmaxPerimeter_= ui->lineEdit_20->text().toInt();
}

void AttributeBased_cd::on_radioButton_12_clicked()
{
    attribute="perimeter";
    att=8;
    int t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.perimeter;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_13_clicked()
{
    attribute="contrast";
    att=6;
    float t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.contrast;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_14_clicked()
{
    attribute="roughness";
    att=6;
    float t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.roughness;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_15_clicked()
{
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.coarseness;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_radioButton_16_clicked()
{
    attribute="direction";
    att=6;
    float t;
    ...
    for(int i = 0 ; i < i2.NO ;i++){
        t=i2.objectArray[i].fVector.direction;
        if(t>max)max=t;
        if(t<min)min=t;
    }
...
void AttributeBased_cd::on_lineEdit_21_returnPressed()
{
    rminContrast_= ui->lineEdit_21->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_22_returnPressed()
{
    rmaxContrast_= ui->lineEdit_22->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_23_returnPressed()
{
    rminRoughness_= ui->lineEdit_23->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_24_returnPressed()
{
    rmaxRoughness_= ui->lineEdit_24->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_25_returnPressed()
{
    rminCoarseness_= ui->lineEdit_25->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_26_returnPressed()
{
    rmaxCoarseness_= ui->lineEdit_26->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_27_returnPressed()
{
    rminDirection_= ui->lineEdit_27->text().toFloat();
}

void AttributeBased_cd::on_lineEdit_28_returnPressed()
{
    rmaxDirection_= ui->lineEdit_28->text().toFloat();
}

void AttributeBased_cd::on_pushButton_2_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxDirection);
    ui->lineEdit_28->setText(text_number);
...
void AttributeBased_cd::on_pushButton_3_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxCoarseness);
    ui->lineEdit_26->setText(text_number);
...
void AttributeBased_cd::on_pushButton_4_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxRoughness);
    ui->lineEdit_24->setText(text_number);
...
void AttributeBased_cd::on_pushButton_5_clicked()
{
    QString text_number;
...
void AttributeBased_cd::on_pushButton_6_clicked()
{
    QString text_number;
...
void AttributeBased_cd::on_pushButton_7_clicked()
{
    QString text_number;
...
void AttributeBased_cd::on_pushButton_8_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxStdGreen);
    ui->lineEdit_16->setText(text_number);
...
void AttributeBased_cd::on_pushButton_9_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxStdRed);
    ui->lineEdit_14->setText(text_number);
...
void AttributeBased_cd::on_pushButton_10_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxMeanBlue);
    ui->lineEdit_12->setText(text_number);
...
void AttributeBased_cd::on_pushButton_11_clicked()
{
    QString text_number;
...
void AttributeBased_cd::on_pushButton_12_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxMeanRed);
    ui->lineEdit_8->setText(text_number);
...
void AttributeBased_cd::on_pushButton_13_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxCompactness);
    ui->lineEdit_6->setText(text_number);
...
void AttributeBased_cd::on_pushButton_14_clicked()
{
    QString text_number;
...
void AttributeBased_cd::on_pushButton_15_clicked()
{
    QString text_number;
    text_number = text_number.setNum(maxArea);
    ui->lineEdit_2->setText(text_number);
...
void AttributeBased_cd::on_pushButton_16_clicked()
{
    QString text_number;
