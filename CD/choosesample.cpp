...
chooseSample::chooseSample(int ncls,int x,int y, int **labels1,int img, QWidget *tempParent,QWidget *parent) :cls_no(ncls),X(x),Y(y),labels1_(labels1),Img(img),tempParent(tempParent),
    QWidget(parent),
    ui(new Ui::chooseSample)
{
    ...
    ui->setupUi(this);
    this->setWindowTitle("Choose Samples");
...
void chooseSample::on_pushButton_clicked()
{
    ...
    tmp->insertvalue(cls_no,X,Y,labels1_,Img);
    this->close();
}
