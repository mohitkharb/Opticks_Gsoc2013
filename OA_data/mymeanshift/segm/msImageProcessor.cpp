...
void msImageProcessor::RGBtoLUV(byte *rgbVal, float *luvVal)//this copy is for Lab conversion
{
	...
	//convert RGB to XYZ...
	double R,G,B;
	//printf("intialvalues - %d %d %d\n",rgbVal[0],rgbVal[1],rgbVal[2]);
	
	R=1.0*rgbVal[0]/255;
	G=1.0*rgbVal[1]/255;
	B=1.0*rgbVal[2]/255;
	R=(((R) <= 0.0404482362771076) ?((R)/12.92) : pow(((R) + 0.055)/1.055, 2.4));
	G=(((G) <= 0.0404482362771076) ?((G)/12.92) : pow(((G) + 0.055)/1.055, 2.4));
	B=(((B) <= 0.0404482362771076) ?((B)/12.92) : pow(((B) + 0.055)/1.055, 2.4));
	//printf("change1 - %f %f %f\n",R,G,B);
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	y  = 1.0*y/1.0;
	z  = 1.0*z/1.088754;		
	x = (x >= 8.85645167903563082e-3) ? pow(x,0.333333333333333) : ((841.0/108.0)*(x) + (4.0/29.0));
	y = (y >= 8.85645167903563082e-3) ? pow(y,0.333333333333333) : ((841.0/108.0)*(y) + (4.0/29.0));
	z = (z >= 8.85645167903563082e-3) ? pow(z,0.333333333333333) : ((841.0/108.0)*(z) + (4.0/29.0));
	//printf("changes in XYZ - %f %f %f\n",x,y,z);


	luvVal[0] = (float)(116*y - 16);
	luvVal[1] = (float)(500*(x - y));
	luvVal[2] = (float)(200*(y - z));
	//printf("Luv values - %f %f %f \n",luvVal[0],luvVal[1],luvVal[2]);
	return;
	
}

/*void msImageProcessor::RGBtoLUV(byte *rgbVal, float *luvVal)//this copy is for sending RGB as it is....
{
...
/*void msImageProcessor::RGBtoLUV(byte *rgbVal, float *luvVal)//this copy is for Luv conversion..
{
...
void msImageProcessor::LUVtoRGB(float *luvVal, byte *rgbVal)//this code is for LABtoRGB
{
...
//--------------------------------------------------------LAB------------------------------------------------------------------
		...
		//printf("lab values passed to conv - %f %f %f \n",L,a,bl);
		float min;
		L = (L + 16)/116;
		a = L + a/500;
		bl = L - bl/200;
		//printf("lab values after change1 - %f %f %f\n",L,a,bl);

		a=((a >= 0.206896551724137931) ?  ((a)*(a)*(a)) : (108.0/841.0)*((a) - (4.0/29.0)));
		bl=((bl >= 0.206896551724137931) ?  ((bl)*(bl)*(bl)) : (108.0/841.0)*((bl) - (4.0/29.0)));
		L=((L >= 0.206896551724137931) ?  ((L)*(L)*(L)) : (108.0/841.0)*((L) - (4.0/29.0)));

		//printf("lab values after change2 - %f %f %f \n",L,a,b);
		...
		//printf("lab to xyz - %f %f %f \n",x,y,z);
//convert xyz to rgb...
		tr  = ((RGB[0][0]*x + RGB[0][1]*y + RGB[0][2]*z));
		tg  = ((RGB[1][0]*x + RGB[1][1]*y + RGB[1][2]*z));
		tb  = ((RGB[2][0]*x + RGB[2][1]*y + RGB[2][2]*z));

		//printf("xyz to trgb - %f %f %f \n",tr,tg,tb);
		
		

		// Transform from RGB to R'G'B' 
		tr = (((tr) <= 0.0031306684425005883) ? (12.92*(tr)) : (1.055*pow((tr), 0.416666666666666667) - 0.055));
		tg = (((tg) <= 0.0031306684425005883) ? (12.92*(tg)) : (1.055*pow((tg), 0.416666666666666667) - 0.055));
		tb = (((tb) <= 0.0031306684425005883) ? (12.92*(tb)) : (1.055*pow((tb), 0.416666666666666667) - 0.055));

		//printf("trgb values after change - %f %f %f \n",tr,tg,tb);
...
/*void msImageProcessor::LUVtoRGB(float *luvVal, byte *rgbVal)//this code is for sending RGBtoRGB....
{
...
/*void msImageProcessor::LUVtoRGB(float *luvVal, byte *rgbVal)//this is for LUVtoRGB
{
...
//	    		mylab[k]=modes[(N*label)+k];
	//		printf("%f\n",modes[(N*label)+k]);
			}
//			LUVtoRGB(mylab,myrgb);
//			for(k = 0; k < N; k++)
//				printf("%d\n",myrgb[k]);
...
//			printf("%d\n",i);
	...
	///////////////////////////////////////////////////////////////////////////////////
	FILE *f1234;
//	f1234=fopen("/home/lsi/Ankush/DEAL_project/seg_images/map_label_connecting_region.txt","w");
    f1234=fopen("C:/Windows/Temp/map_label_connecting_region.txt","w");
	for(i = 0; i < height*width; i++)
			fprintf(f1234,"%d\n",labels[i]);
	fclose(f1234);
	///////////////////////////////////////////////////////////////////////////////////
...
void msImageProcessor::TransitiveClosure( void )
{
	...
	FILE *f12345;
//	f12345=fopen("/home/lsi/Ankush/DEAL_project/seg_images/map_label_transitive_closure.txt","w");
    f12345=fopen("C:/Windows/Temp/map_label_transitive_closure.txt","w");
...
void msImageProcessor::Prune(int minRegion)
{
	...
	do
	{
		...
		int temp_l=1;
		FILE *f123;
	//	printf("%s",inputImage_);

	//	f123=fopen("/home/lsi/Ankush/DEAL_project/seg_images/map_label_pruning_spurious_regions.txt","w");
        f123=fopen("C:/Windows/Temp/map_label_pruning_spurious_regions.txt","w");
		//fprintf(f123, "NCOLS %d\n NROWS %d\n CELLSIZE %d \n XLLCORNER %d \n YLLCORNER %d \n",width,height,1,0,0);
		for(i = 0; i < height*width; i++){
